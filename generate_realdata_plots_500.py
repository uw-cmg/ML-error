...
for dataset in datasets:
    for model in models:
        ...
        if dataset == "Perovskite":
            ...
            print("Valid dataset not provided in generate_realdata_plots.py")
        ...
        CVD = cvd.CVData()
        CV_residuals, CV_model_errors = CVD.get_residuals_and_model_errors(model, X_train_split, y_train_split, model_num=500)
        ...
        TD = td.TestData()
        Test_residuals, Test_model_errors_unscaled, Test_model_errors_scaled, a_array, b_array = TD.get_residuals_and_model_errors_looped(dataset, model, X_train, y_train, model_num=500)
        print('a_array:')
        print(a_array)
        print('b_array:')
        print(b_array)
